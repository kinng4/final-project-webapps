...
#Register path calls the register form template and creates a User1 object
get '/register' do
  @title = "Register"
  slim :register
end
...
post '/register' do
  ...
  user = User1.get(params[:username].downcase)
  ...
  if user
    ...
    User1.create(username: username.downcase, password: hash_password, role: params[:role])
    redirect to("/login")
  end
end
...
get '/uploadcsv' do
  @title = "Upload CSV"
  if session[:id]
    @instructor = User1.get(session[:id].username)
...
post '/uploadcsv' do
  ...
  CSV.foreach(target, :headers => true) do |row|
    user = User1.get(row[0].downcase)
    hash_password = BCrypt::Password.create(row[1])
    User1.create(username: row[0].downcase, password: hash_password, role: row[2]) unless user
